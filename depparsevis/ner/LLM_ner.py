...
class OllamaNER:
    ...
    def __init__(self, host: str):
        ...
        self.default_prompt = """You are a named entity recognition (NER) system. Your task is to process the following text and identify all occurrences of entities belonging to the following types: MODEL, APPLICATION, and LICENCE.

For each identified entity, output an object with the following properties:
- "text": The exact text of the entity.
- "type": The entity type, which must be one of MODEL, APPLICATION, or LICENCE.
- "word_start": The index of the first word of the entity (with the first word of the text having index 0).
- "word_end": The index of the last word of the entity (inclusive).
- "char_start": The character index where the entity begins.
- "char_end": The character index where the entity ends.
- "score": A confidence score as a float (between 0.0 and 1.0).

Return your response strictly in valid JSON format following this schema:
{{
  "entities": [
    {{
      "text": "EntityText",
...
**Instructions:**
1. Do not include any extra keys or formatting outside the specified JSON structure.
2. Only extract entities that belong to the types MODEL, APPLICATION, or LICENCE.
...
The word list is provided below as a dictionary where each key is a word index and the value is a tuple (word_text, start_char_index, end_char_index):
---
{word_list}
---
"""
    ...
    def _transform_entity_to_labelstudio(
    ...
    def predict(
        ...
        for text in texts:
            ...
            words_dict = self._split_into_words(text)

            # For the prompt, provide the dictionary (keys will be serialized as strings).
            content = prompt.format(text=text, word_list=json.dumps(words_dict))
            ...
            try:
                ...
                for entity in ner_response.entities:
                    transformed = self._transform_entity_to_labelstudio(
                        text, entity, words_dict
                    )
...
if __name__ == "__main__":
    ...
    test_sentences = [
        "The Llama 2 model was integrated into the SummarizeAI application under the Apache 2.0 licence.",
        "The Pythia-6.9b-v0 model, a large-scale language model with 6.9 billion parameters, is particularly suited for text-generation tasks, leveraging its autoregressive architecture to produce coherent and contextually relevant outputs across diverse domains; released under the Apache-2.0 license, it provides researchers and developers with a permissive framework for modification, redistribution, and deployment, thereby fostering innovation and reproducibility in AI/ML research while ensuring compliance with open-source licensing standards.",
    ]

    model_name = "llama3.1:latest"

    result = ner_model.predict(
        test_sentences, model_name=model_name, temperature=0.0, top_k=40, top_p=0.8
    )
